...
taken = []
for i in range(0,length):
    taken.append(li[i][li[i].apply(lambda r: r.str.contains('Taken', case=False).any(), axis=1)])

open = []
for i in range(0,length):
    open.append(li[i][li[i].apply(lambda r: r.str.contains('Open', case=False).any(), axis=1)])   
